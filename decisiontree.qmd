...
$R(t)$ provides a link between reported cases of a disease and many additional dynamic processes, including disease characteristics (e.g., infectiousness under various conditions, mode of transport) and extrinsic factors (e.g., lockdowns that reduce person-to-person contact). It represents "... the expected number of new infections caused by an infectious individual in a population where some individuals may no longer be susceptible" (@Gostic2020Practical), and has two main uses:
...
Future efforts include expanding this table to include **alternative data sources** (e.g., wastewater) and packages in **other coding languages** (e.g., Python).
:::
