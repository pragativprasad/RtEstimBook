...
In this paper we provide a theoretical comparison of the current field of methods for estimating $R(t)$, with the goal of informing user decision-making about which package to choose and in interpreting package outputs. @tbl-packages lists packages with an accompanying peer-reviewed journal manuscript, Table 2 lists packages without a peer-reviewed journal manuscript, and Table S1 contains R(t) packages that calculate $R(t)$ but were excluded from this summary. In the text, the package citation is given the first time each package is referenced.
